  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const validated: ValidationResult[] = rows.map((row, index) => {
        ...
        if (!row.teacher_email?.trim()) {
          errors.push("teacher_email tidak boleh kosong");
        }
        if (!row.subject_code?.trim()) {
          errors.push("subject_code tidak boleh kosong");
        }
                ...
                <tbody className="divide-y">
                  {previewRows.map((row) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-2 text-xs">{row.teacher_email}</td>
                      <td className="px-3 py-2 text-xs">{row.subject_code}</td>
